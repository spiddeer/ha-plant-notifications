...
This blueprint offers flexible notification options:

### Easy Setup - Mobile App
Simply select your mobile device from the dropdown. Notifications will be sent automatically with no additional configuration needed.

### Advanced Setup - Other Services (Optional)
Add custom notification actions for:
- **Telegram** - `service: notify.telegram`
- **Email** - `service: notify.email`
- **Multiple devices** - Add multiple notify actions
- **Custom formatting** - Full control over message format

You can use mobile app notifications, custom actions, or both together!
...
1. **When moisture drops below threshold:**
   ...
   - Send mobile app notification (if device selected)
   - Run additional notification actions (if configured)
   - Create persistent notification in HA

2. **When moisture rises above threshold (and boolean is on):**
   - Send "watered" confirmation notification
   - Run additional notification actions (if configured)
